...
const VisuallyHiddenInput = styled('input')({
    ...
    const encryptedData = DataEncrypt(JSON.stringify(all_parameters));
      ...
      const getCategories = async () => {
        try {
          const response = await api.post("/api/miscellaneous/get-category", reqData);
      ...
      const handleSubmit = async () => {
        ...
        try {
          ...
          const response = await api.post('/api/miscellaneous/add-course-video', formData,{
            ...
            <Grid item={true} xs={12}   >
                <TableContainer component={Paper} >
                    ...
                    <Box display={'inline-block'} justifyContent={'space-between'} alignItems={'right'} mt={1} mb={1} style={{width: '30%', verticalAlign: 'top'}} >
                        <Typography variant="h5"  sx={{ padding: 2 }}>Add New Marketing Video</Typography>
                    </Box>
...
export default AddVideoTransactions;