...
export default function KYCStatusModal({filterData, setFilterData,  KYCStatus, KYCStatusClose }) {

    const [stateArray, setStateArray] = React.useState([
        // {name : "All" , checked: true},
        {name : "Auto Accepted" , checked: true},
        {name:"Auto Rejected",checked : true},
        {name:"Not Applied",checked : true},
    ])
...
{stateArray?.map((v,id) =><FormControlLabel labelPlacement="start" key={id}  control={<Checkbox checked={v?.checked} onClick={(e) => setStateArray(stateArray?.map(k => k?.name == v?.name ? {name : k?.name ,checked: e.target.checked} : k))}/>} label="" />)}
                    ...
                    <Box width={"100%"} display={"flex"} justifyContent={"space-around"} mt={4}>
                        <Box style={{ cursor: "pointer", padding: "4px 19px", border: "2px solid #FF9F59" }} borderRadius={"4px"} width={"120px"} mr={1} onClick={KYCStatusClose}>
                            <Typography color={"#FF9F59"} fontSize={"18px"} textAlign={"center"} fontWeight={500}>Reset</Typography>
                        </Box>
                        <Box style={{ cursor: "pointer", padding: "4px 19px", backgroundColor: "#FF9F59" }} borderRadius={"4px"} width={"120px"} ml={1} onClick={KYCStatusClose}>
                            <Typography color={"white"} fontSize={"18px"} textAlign={"center"} fontWeight={500}
onClick={() => setFilterData({...filterData , kyc_status : stateArray?.filter(v => v?.checked)?.map(k => k?.name)})}
