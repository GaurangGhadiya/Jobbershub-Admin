...
export default function TravelFundsModal({filterData,setFilterData, travelFunds, travelFundsClose }) {
    ...
    return (
        <React.Fragment>
            ...
            {/* <BootstrapDialog
                onClose={travelFundsClose}
                open={pendingLeads}
            > */}


            {travelFunds && <ClickAwayListener onClickAway={travelFundsClose}>
                ...
                <Box>
                    <Box display={"flex"} flexWrap={"wrap"}>
                                {box.map(v => (<Box key={v?.title} borderRadius={"4px"} padding={"7px 10px"} textAlign={"center"} mr={2} mb={2}
                                 style={{ cursor: "pointer", border : v?.value == internalState?.travel_income_filter ? "1px solid #ED6A0F" :"1px solid black",

                                    color : v?.value == internalState?.travel_income_filter ? "#ED6A0F" :"black",
                                  }}
                                    onClick={() => setInternalState({ ...internalState, travel_income_filter : v?.value})}
                        >
                                <Typography>{v?.title}</Typography>
                            </Box>))}
                        </Box>
                        {internalState?.travel_income_filter == "custom" &&  <LocalizationProvider dateAdapter={AdapterDayjs}>
                ...
                <Box display={"flex"} justifyContent={"space-between"} alignItems={"center"} mt={2}>
                        <Box display={"flex"} justifyContent={"space-between"} >
                            ...
                            <input type='number' value={internalState?.travel_income_above} onChange={(e) => setInternalState({...internalState, travel_income_above : e.target.value })} style={{ borderRadius: "2px", border: "1px solid #000000", padding: "5px 10px", fontSize: "16px", width: "120px" }} />
                        </Box>
                        <Box display={"flex"} justifyContent={"space-between"}>
                            <Typography mr={1} mt={0.5}>Below</Typography>
                            <input type='number' value={internalState?.travel_income_below}  onChange={(e) => setInternalState({...internalState, travel_income_below : e.target.value })} style={{ borderRadius: "2px", border: "1px solid #000000", padding: "5px 10px", fontSize: "16px", width: "120px" }} />
                        </Box>
                    </Box>


                <Box width={"100%"} display={"flex"} justifyContent={"end"} mt={4}>
                    <Box style={{ cursor: "pointer", padding: "7px 19px", border: "2px solid #FF9F59" }} borderRadius={"4px"} width={"120px"} mr={1} onClick={travelFundsClose}>
                        <Typography color={"#FF9F59"} fontSize={"18px"} textAlign={"center"} fontWeight={500}>Reset</Typography>
                    </Box>
                    <Box style={{ cursor: "pointer", padding: "7px 19px", backgroundColor: "#FF9F59" }} borderRadius={"4px"} width={"120px"} ml={1} onClick={travelFundsClose}>
