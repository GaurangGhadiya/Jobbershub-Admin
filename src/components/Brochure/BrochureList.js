...
const BrochureTransactions = ({ showServiceTrans }) => {
    ...
    const handleCloseModal2 = () => {
      ...
      const handleOKButtonClick = async () => {
        ...
        if (!Id) {
          console.error('Brochure Id is missing.');
          return;
        }
        ...
        try {
            ...
            const response = await api.post("/api/miscellaneous/update-brochure-status", requestData);
                        ...
                        <Table aria-label="User Details" sx={{ size: 2 }} mt={2}>
                            <TableHead>
                                <TableRow>
                                    ...
                                    <StyledTableCell style={{ fontWeight: 'bold', whiteSpace: 'nowrap' }} >Brochure Name</StyledTableCell>
                                    <StyledTableCell style={{ fontWeight: 'bold', whiteSpace: 'nowrap' }} >PDF File</StyledTableCell>
                            ...
                            <TableBody>
                                {showServiceTrans.length > 0 ? (rowsPerPage > 0
                                    ...
                                    >
                                        ...
                                        <StyledTableCell>{row.brochure_name}</StyledTableCell>
                                        <StyledTableCell> {row.brochure_file !== '' ? (
                                            <Link href="#" onClick={() => handleLinkClick(row.brochure_file)}>View PDF</Link>
                                        ) : (
                                            ''
                                        )}
                                       </StyledTableCell>
                                       <StyledTableCell> {row.image !== '' ? (
                                        ...
                                        <StyledTableCell sx={{ '& button': { m: 1 } }}>
                                            <Link href={`/update-brochure/?id=${row.id}`}>
                                            <a>
                                            ...
                                            {row.status === 0 || row.status === 2 ? (
                                                ...
                                                <>
                                                    <Button variant="contained" size="small" color="warning" onClick={() => handleOpenModal1(row.id, 2)}>Inactive</Button> 
                                                ...
                                                >
                                                    ...
                                                    <Typography id="modal-modal-title" variant="h6" component="h2">
                                                     Are you sure to active/inactive Brochure?
                                                    </Typography>
                                                ...
                                                >
                                                    <Box sx={style} alignItems={'center'}  justifyContent={'space-between'}>
                                                        <HelpOutlineOutlinedIcon sx={{ fontSize: 40 ,marginLeft:20}} color="warning" alignItems={'center'} />
                                                    <Typography id="modal-modal-title" variant="h6" component="h2">
                                                     Are you sure to Delete this brochure?
                                                    </Typography>
                                                    {/* <TextareaAutosize 
...
export default BrochureTransactions;