...
import ParentCategoryTable from "../components/Product/main_category"; // Adjust the import path
import SubCategoryTable from "../components/Product/sub_category"; // Adjust the import path
import ChildCategoryTable from "../components/Product/child_category"; // Adjust the import path
import ProductTable from "../components/leads/leads_report"; // Adjust the import path
...
function Dashboard1(props) {
    ...
    const [activeCategory, setActiveCategory] = useState('parent'); // State to track the active category
    ...
    const getCategories = async () => {
      ...
      const getTnx = async () => {
        ...
        try {
          ...
          if (response.status === 200) {
            setProducts(response.data.data); // Assuming this is where you get your products
          }
        } catch (error) {
      ...
      useEffect(() => {
        // Show products by default when the component mounts
        getTnx();
      }, []);
    ...
    return (
      ...
      <Button variant="contained" onClick={() => setActiveCategory('parent')} style={{ marginRight: "10px" }}>
        Show Parent Categories
      </Button>
      <Button variant="contained" onClick={() => setActiveCategory('sub')} style={{ marginRight: "10px" }}>
        Show Sub Categories
      </Button>
      <Button variant="contained" onClick={() => setActiveCategory('child')} style={{ marginRight: "10px" }}>
        Show Child Categories
      </Button>
      <Button variant="contained" onClick={getTnx} style={{ marginRight: "10px" }}>
        Show Products
      </Button>
