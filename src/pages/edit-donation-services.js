...
function TransactionHistory(props) {
    ...
    useEffect(() => {
        const getTnx = async () => {
          ...
          try {
            const response = await api.post('/api/log/get-donation-services-by-id', reqData);
            if (response.status === 200) {
                setservice_name(response.data.data.title);
                setstatus(response.data.data.status);
                setcategory_image(response.data.data.image_url);
            }
          } catch (error) {
    ...
    const handleSubmit = async () => {
        ...
        try {
            const response = await api.post("/api/log/update-donation-service", formData, {
            ...
            <Grid item={true} xs={12}   >
              <TableContainer component={Paper} >
                <Box display={'inline-block'} justifyContent={'space-between'} alignItems={'right'} mt={1} mb={1} style={{width: '40%', verticalAlign: 'top'}} >
                    <Typography variant="h5"  sx={{ padding: 2 }}>Donations And Devotion Services [Update] </Typography>
                </Box>
                </TableContainer>
            </Grid>
                ...
                <Grid item={true} xs={12}   >
                    <TableContainer component={Paper} >
                        ...
                        <Box justifyContent={'space-between'} alignItems={'right'} mt={1} mb={1} style={{width: '50%', verticalAlign: 'top', padding: '0 10px'}} >
                            ...
                            value={service} onChange={(e) => setservice_name(e.target.value)} />
                        </Box>
                        <br/>
