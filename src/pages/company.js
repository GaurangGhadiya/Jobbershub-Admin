...
import Transactions from "@/components/userManagement/company";
...
import AddVendorForm from '@/components/userManagement/add-company'; 
...
const TransactionHistory = (props) => {
  ...
  useEffect(() => {
      const getTnx = async () => {
          const reqData = {};
          try {
              const response = await api.get('/api/employee/get-company-list', reqData);
          ...
          <Grid container spacing={4} >
              <Grid item xs={12}>
                <Box display="flex" justifyContent="space-between" alignItems="center" mt={1} mb={1} style={{ width: '100%' }}>
                    ...
                    <Button variant="contained" color="success" sx={{marginRight:"10px"}} size="medium" onClick={handleOpenForm}>
                    <AddIcon />Add Company
                    </Button>
                </Box>
              </Grid>
