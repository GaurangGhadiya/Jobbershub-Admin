...
function TransactionHistory(props) {
    ...
    useEffect(() => {
        const getTnx = async () => {
          ...
          try {
            const response = await api.post('/api/miscellaneous/get-video-category', reqData);
            if (response.status === 200) {
    ...
    const handleSubmit = async () => {
        ...
        try {
            const response = await api.post("/api/miscellaneous/update-video-category", formData);
