...
import Transactions from "@/components/Donations/donation";
...
function TransactionHistory(props) {
    ...
    useEffect(() => {
        ...
        const getTnx = async () => {
          ...
          try {
            const response = await api.post('/api/log/get-donation-services');
            if (response.status === 200) {
              const decryptedObject = response.data;
            ...
            <Grid item={true} xs={12}   >
              <TableContainer component={Paper} >
                <Box display={'inline-block'} justifyContent={'space-between'} alignItems={'right'} mt={1} mb={1} style={{width: '70%', verticalAlign: 'top'}} >
                    <Typography variant="h5"  sx={{ padding: 2 }}>Donations And Devotion Services</Typography>
                </Box>
