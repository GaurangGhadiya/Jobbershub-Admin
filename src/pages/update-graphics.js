...
function TransactionHistory(props) {
    ...
    useEffect(() => {
        const getCategories = async () => {
          try {
            const response = await api.get("/api/graphics/get-category-list-admin");
    ...
    useEffect(() => {
        ...
        //   };
          try {
            const response = await api.post('/api/graphics/get-graphic', all_parameters);
          
            if (response.status === 200) {
                // const decryptedObject = DataDecrypt(response.data);
             
                setTitle(response.data.data.graphics_name);
                setCatGroup(response.data.data.cat_group);
    ...
    const handleSubmit = async () => {
        ...
        try {
            if(!allowedCharacters.test(cat_group)){
                ...
                const response = await api.post('/api/graphics/update-graphics', formData,{
        ...
        <Layout>
            ...
            >
               ...
               <Grid item={true} xs={12}   >
                    ...
                    <Typography variant="h5"  sx={{ padding: 2 }}>Marketing [Update] </Typography>
                    </Box>

                    <Box justifyContent={'space-between'} alignItems={'right'} mt={1} mb={1} style={{width: '50%', verticalAlign: 'top', padding: '0 10px'}} >
                        
                        <TextField required  fullWidth label="Graphics Name" variant="outlined" display={'inline-block'}
                    ...
                    <Box justifyContent={'space-between'} alignItems={'right'} mt={1} mb={1} style={{width: '50%', verticalAlign: 'top', padding: '0 10px'}} >
                        ...
                        <FormControl fullWidth>
                            <InputLabel id="demo-simple-select-label">Graphics Category</InputLabel>
                                    <Select
                    ...
                    <Box justifyContent={'space-between'} alignItems={'right'} mt={1} mb={1} style={{width: '50%', verticalAlign: 'top', padding: '0 10px'}} >
                        ...
                        <TextField required  fullWidth label="Graphics Category Group" variant="outlined" display={'inline-block'}
                        value={cat_group} onChange={(e) => setCatGroup(e.target.value)}  />
                    </Box>
