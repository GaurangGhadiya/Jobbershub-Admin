...
import Transactions from "@/components/CreditCard/credit_card";
...
function TransactionHistory(props) {
    ...
    useEffect(() => {
        ...
        const getTnx = async () => {
          ...
          try {
            const response = await api.post('/api/leads/get-credit-card', reqData);
            ...
            <Grid item={true} xs={12}   >
              <TableContainer component={Paper} >
                <Box display={'inline-block'} justifyContent={'space-between'} alignItems={'right'} mt={1} mb={1} style={{width: '70%', verticalAlign: 'top'}} >
                    <Typography variant="h5"  sx={{ padding: 2 }}>Credit card [List]</Typography>
                </Box>
