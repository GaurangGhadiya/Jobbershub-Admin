...
function LeadsDetailsHistory(props) {
    ...
    useEffect(() => {
        const getTnx = async () => {
          ...
          try {
            const response = await api.post('/api/leads/get-leads-details-report', reqData);
    ...
    return (
        ...
        <Layout>
            ...
            <Grid item={true} xs={12}   >
              <TableContainer component={Paper} >
                ...
                <Box display={'inline-block'} justifyContent={'space-between'} alignItems={'right'} mt={1} mb={1} style={{width: '20%', verticalAlign: 'top'}} >
                    <Typography variant="h5"  sx={{ padding: 2 }}>Leads Details List</Typography>
                </Box>
            ...
            <LeadDetailsTransactions showServiceTrans={showServiceTrans} />
        </Layout>


    );
}
export default withAuth(LeadsDetailsHistory);
