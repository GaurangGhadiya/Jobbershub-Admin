...
import Transactions from "@/components/marketingVideo/VideoList";
...
function TransactionHistory(props) {
    ...
    useEffect(() => {
        ...
        const getTnx = async () => {
          ...
          try {
            const response = await api.post('/api/miscellaneous/get-videos-admin', reqData);
            ...
            <Grid item={true} xs={12}   >
              <TableContainer component={Paper} >
                <Box display={'inline-block'} justifyContent={'space-between'} alignItems={'right'} mt={1} mb={1} style={{width: '20%', verticalAlign: 'top'}} >
                    <Typography variant="h5"  sx={{ padding: 2 }}>Marketing Video List</Typography>
                </Box>
                ...
                <Box display={'inline-block'} justifyContent={'space-between'} alignItems={'center'} mt={2} mb={1} sx={{ verticalAlign: 'middle' }}>
                    ...
                    {/* <DemoContainer  ml={2} components={['DatePicker', 'DatePicker']}  > */}
                        ...
                        href={`/add-new-marketing-video/`}
                      >
                        Add New
                    </Button>   
                    <Button variant="contained"  href={`/marketing-video-category/`} >Category List </Button>
                    </Box>
                        </TableContainer>
            </Grid>
