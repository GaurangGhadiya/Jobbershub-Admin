...
import Transactions from "@/components/userManagement/employee-assets";
import { Grid,Paper,TableContainer, Button } from "@mui/material";
import { Typography,Divider,Box,TextField} from "@mui/material";


import AddVendorForm from '@/components/userManagement/add-role'; 
...
const TransactionHistory = (props) => {
  ...
  useEffect(() => {
      const getTnx = async () => {
          const reqData = {};
          try {
              const response = await api.post('/api/employee/get-emp-assets', reqData);
  ...
  return (
    ...
    <Box display="flex" justifyContent="space-between" alignItems="center" mt={1} mb={1} style={{ width: '100%' }}>
        <Typography variant="h5" sx={{ padding: 2 }}>Employee assets</Typography>
        <Button variant="contained" color="success" size="medium" sx={{marginRight:"10px"}} onClick={handleOpenForm}>
            Add Assets
        </Button>
    </Box>
</TableContainer>
