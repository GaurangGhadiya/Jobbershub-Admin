...
import Transactions from "@/components/AssetManagement/asset_category";
import { Grid,Paper,TableContainer, Button } from "@mui/material";
import { Typography,Divider,Box,TextField} from "@mui/material";
import AddVendorForm from '@/components/AssetManagement/add_asset_category'; 
...
const TransactionHistory = (props) => {
  ...
  useEffect(() => {
      ...
      const getTnx = async () => {
          const reqData = {};
          try {
              const response = await api.post('/api/employee/get-asset-category-list', reqData);
          ...
          <Grid container spacing={4} >
            <Grid item xs={12}>
                <Box display="flex" justifyContent="space-between" alignItems="center" mt={1} mb={1} style={{ width: '100%' }}>
                    <Typography variant="h5" >Asset Category List</Typography>
